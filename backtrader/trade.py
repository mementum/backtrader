...
class Trade(object):
    ...
    def __init__(self, data=None, tradeid=0, historyon=False,
        ...
        self.R = R  # ROR added..
        #print("TRADE init  ############################ R defined to be .... ", self.R)
    ...
    def update(self, order, size, price, value, commission, pnl,
        ...
        '''
        #print('Trade update()') #ROR
        if not size:
            return  # empty update, skip all other calculations
        #print('Trade update() passed initial test') #ROR
        ...
        if abs(self.size) > abs(oldsize):
            ...
            print('*********  Trade here reduced/closed   *******') #ROR
            print('price = ',price)
            ...
            if self.R == None: # ROR
                ...
                pnl = comminfo.profitandloss(-size, self.price, price)
                risk = abs(self.R - self.price) # ROR self.price = inital entry price, price = exit price
                pnl = pnl/risk
                print("\n*********** Trade update   R =", self.R)
            #self.cunt = order.info  # ROR added: test: info is where kwargs are stored
            #qq='*'*52 # ROR
            #qqq=(qq + str(order.info) + '\n')*52 # ROR
            #print(qqq) # ROR  test we get here
